...
describe('reverse()', function() {
  it('should reverse the string provided', function() {
    var test = "good times";
    expect(quiz.reverse(test)).to.eql('semit doog');
  });
});

describe('makePizza()', function() {
  it('should return a Pizza object with an ingredients array and a size property', function() {
    var pizza = new quiz.makePizza();
    expect(pizza.constructor.name).to.eql('Pizza');
    expect(Array.isArray(pizza.ingredients)).to.eql(true);
    expect(pizza.size).to.eql('large');
  });
});

describe('myCar()', function() {
  it('should create a Car object', function() {
    var car = new quiz.myCar();
    expect(car.constructor.name).to.eql('Car');
  })

  it('creates a Car with a drive method', function() {
    var car = new quiz.myCar();
    car.drive(100);
  });

  it('creates a Car with a driven method', function() {
    var car = new quiz.myCar();
    car.drive(100);
